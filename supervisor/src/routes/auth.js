...
const { requireAuth, principalFromUser, isPanelAdmin } = require('../auth');
...
router.post('/invite/:token', inviteLimiter, async (req, res) => {
  try {
    ...
    finalizeSession(req, res, row, { remember: false, status: 201, extra: { platform_role: user.platform_role } });
  } catch (e) {
...
function finalizeSession(req, res, user, { remember = false, status = 200, extra = {} } = {}) {
  req.session.regenerate(err => {
    ...
    req.session.save(err2 => {
      ...
      res.status(status).json({ ok: true, role: user.role, username: user.username, ...extra });
    });
  });
}
...
router.get('/me', (req, res) => {
  ...
  if (requireTotpAdmins && isPanelAdmin(p) && !user.totp_secret) {
    return res.json({ ...base, totp_setup_required: true });
  }

  res.json({ ...base, needsOnboarding: computeNeedsOnboarding(user) });
});
